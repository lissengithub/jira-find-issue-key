...
function allMatches(prefix, strings) {
...
async function findStrings() {
    ...
    if (github.context.payload.pull_request &&
        ...
        promises.push(oktokit.pulls
            .get({
            owner: github.context.payload.repository.owner.login,
            repo: github.context.payload.repository.name,
            pull_number: github.context.payload.pull_request.number,
        })
            .then((response) => result.add(response.data.head.label)));
        promises.push(oktokit.pulls
            .listCommits({
            owner: github.context.payload.repository.owner.login,
            repo: github.context.payload.repository.name,
            ...
            .then((response) => response.data.forEach((rr) => result.add(rr.commit.message))));
    }
